...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Validate type ID
    if (empty(trim($_POST["type_id"]))) {
        $error = "Please enter a type ID.";
    } else {
    ...
    if (empty($error)) {
        $sql = "UPDATE asset_type SET type_name = ? WHERE type_id = ?";
        
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("si", $param_type_name, $param_type_id);
            ...
            if ($stmt->execute()) {
                $_SESSION['message'] = "Asset type updated successfully";
                header("Location: manage_astty.php");
                exit();
            } else {
    ...
    if (isset($_GET["id"]) && !empty(trim($_GET["id"]))) {
        ...
        $sql = "SELECT * FROM asset_type WHERE type_id = ?";
        if ($stmt = $conn->prepare($sql)) {
...
<head>
    ...
    <title>Edit Asset Type - ICT IT Asset Management System</title>
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="type_name" name="type_name" 
                               value="<?php echo htmlspecialchars($type_name); ?>" required>
                    </div>
